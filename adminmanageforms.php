...
<div class="container">
  <h1 class="text-center mb-3 display-3">MANAGE FORMS</h1>
  <h1 class="text-center mt-4 mb-3">DROPDOWN OPTIONS</h1>
  <?php
  if(isset($_SESSION['successoptiondelete']))
  {
    ?>
    <div class="alert alert-success alert-dismissible fade show text-start mt-2" role="alert">
      ...
      <?= $_SESSION['successoptiondelete']; ?>
      <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
    </div>
    <?php 
    unset($_SESSION['successoptiondelete']);
  }
  ?>
  <?php
  if(isset($_SESSION['erroroptiondelete']))
  {
    ?>
    <div class="alert alert-warning alert-dismissible fade show text-start mt-2" role="alert">
      ...
      <?= $_SESSION['erroroptiondelete']; ?>
      <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
    </div>
    <?php 
    unset($_SESSION['erroroptiondelete']);
  }
  ?>
  <?php
  ...
  ?>
  <?php
  if(isset($_SESSION['savedoption']))
  {
    ?>
    <div class="alert alert-success alert-dismissible fade show text-start" role="alert">
      <i class="bi bi-check-circle-fill" width="24" height="24"></i>
      <?= $_SESSION['savedoption']; ?>
      <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
    </div>
    <?php 
    unset($_SESSION['savedoption']);
  }
  ?>
  <div class="col-md-12">
    <div class="card mt-2">
      <div class="card-body text-end">
        <div class="btn-group">
          <a class="btn btn-md btn-success" href="adminaddformsoption.php"><i class="bi bi-plus-circle"></i> Add Option</a>
        </div>
      </div>
    </div>
  </div>
  <div class="col-md-12">
    <div class="card mt-2">
      <h3 class="card-header text-center">BRAND OF VACCINES</h3>
      <div class="card-body">
        <div class="table-responsive">
          <table class="table">
            <thead>
              <tr>
                <th>Vaccine Brands</th>
                <th>Actions</th>
              </tr>
            </thead>
            <tbody>
              <?php
              include 'conn.php';
              $query = "SELECT * FROM vaccine_list";
              ...
              if (mysqli_num_rows($query_run) > 0) {
                foreach ($query_run as $items) {
                  ?>
                  <tr>
                    <td><?= $items['vaccinebrands']; ?></td>
                    <td class="text-right">
                      <a href="admindeleteformsoption.php?id=<?= $items['v_id']; ?>" class="btn btn-danger btn-sm" style="width: 40px;"><i class="bi bi-trash"></i></a>
                    </td>
                  </tr>
                  <?php
                }
